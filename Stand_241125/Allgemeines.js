...
let material = new THREE.MeshStandardMaterial({
    metalness: 0.5, // Definiert, wie metallisch das Material ist (Werte zwischen 0 und 1)
    roughness: 0.5, // Definiert, wie rau die Oberfläche ist (Werte zwischen 0 und 1)
    color: 0xffffff  // Grundfarbe
});
...
dirLight.shadow.mapSize.width = 1024;  // Schattenauflösung
...
loader.load('Assets/Gesamtmodell-v1.glb', function(loadedGltf) {
    loadedGltf.scene.traverse(function(node) {
        if (!node.isMesh) {
            node.removeFromParent(); // Entferne nicht benötigte Nodes
        }
    });
